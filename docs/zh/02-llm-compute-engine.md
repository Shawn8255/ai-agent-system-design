...
这里需要区分 token 数量和计算成本。Prompt 压缩、RAG、索引可以直接减少输入 token；但模型蒸馏不一定减少 token。一个小模型和一个大模型可能吃进去同样多的 token，但小模型的计算成本更低。

因此更准确的说法是：蒸馏主要减少对昂贵大模型的使用，而不是必然减少 token。只有当蒸馏让工作流从多次大模型调用变成一次小模型处理，或者减少了多轮规划过程时，token 总量才可能随之下降。

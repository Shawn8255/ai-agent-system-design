# 中文文档

本目录包含中文 Markdown 版本。第 1-3 章来自原 DOCX 源文件并保持原有含义；第 4-13 章继续沿用同一架构主线写作，形成第一部分完整草稿。

- [前言、目录大纲与术语约定](00-preface.md)
- [第 1 章 为什么 AI Agent 让我想到了经典计算机工程](01-classical-computer-engineering.md)
- [第 2 章 LLM：一种新的 Compute Engine](02-llm-compute-engine.md)
- [第 3 章 Agent：为什么它更像 Orchestrator](03-agent-orchestrator.md)
- [第 4 章 Memory、Tool 与 Planner 在 Agent 中的职责](04-memory-tool-planner.md)
- [第 5 章 Compute / Storage Separation 在 AI 中的体现](05-compute-storage-separation.md)
- [第 6 章 Stateless Agent 与微服务设计](06-stateless-agent.md)
- [第 7 章 Context Engineering、Prompt Index 与 Query Optimization](07-context-engineering-query-optimization.md)
- [第 8 章 RAG、Retrieval 与 Context Routing](08-retrieval-context-routing.md)
- [第 9 章 Token Reduction、Distillation 与 Tiered Compute](09-token-reduction-distillation-tiered-compute.md)
- [第 10 章 Agent 生产可靠性：幂等、状态机与回放](10-agent-production-reliability.md)
- [第 11 章 Multi-Agent、并发调度与多租户](11-multi-agent-concurrent-scheduling.md)
- [第 12 章 Agent 安全：Prompt Injection、Sandbox 与权限边界](12-agent-security.md)
- [第 13 章 未来方向：Agent Operating System](13-agent-operating-system.md)
- [参考资料](98-references.md)
- [后续修订方向](99-future-chapters.md)
- [发布与开源笔记](notes/publication-strategy.md)
